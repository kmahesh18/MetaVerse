...
export function Dashboard() {
  ...
  const fetchAccessibleSpaces = useCallback(async () => {
    if (!user) return [];
    try {
      const response = await fetch(`http://localhost:${import.meta.env.VITE_BKPORT}/api/user/${user.id}/spaces`);
      if (!response.ok) {
      ...
      const spaceDetailsPromises = accessibleSpaceIds.map(async (spaceId) => {
        try {
          const spaceResponse = await fetch(`http://localhost:${import.meta.env.VITE_BKPORT}/api/spaces/${spaceId}`);
          if (!spaceResponse.ok) {
  ...
  const handleEnterSpace = async (spaceId: string) => {
    ...
    try {
      ...
      if (isActive) {
        ...
        const spaceResponse = await axios.get(`http://localhost:${import.meta.env.VITE_BKPORT}/api/spaces/${spaceId}`);
      ...
      const response = await axios.post(`http://localhost:${import.meta.env.VITE_BKPORT}/api/spaces/${spaceId}/join`, {
          clerkId: user.id
      });
  ...
  const handleLeaveSpace = async (spaceId: string) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`http://localhost:${import.meta.env.VITE_BKPORT}/api/spaces/${spaceId}/leave`, {
        clerkId: user.id
      });
