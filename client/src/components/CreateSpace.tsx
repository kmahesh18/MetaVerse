...
export function CreateSpace() {
  ...
  const handleRoomCountChange = useCallback((roomTypeId: string, count: number) => {
    setRoomCounts(prev => ({
      ...prev,
    ...
    <div className="container-2d" style={{ 
      ...
      {error && (
        <div style={{ 
          ...
          <p style={{ color: '#ff0000', margin: 0, fontWeight: 'bold' }}>{error}</p>
        </div>
      )}
      ...
      <div className="room-selection" style={{ marginTop: '20px' }}>
        ...
        <div className="room-types-grid" style={{ 
          display: 'grid', 
          ...
          {roomTypes.map((roomType) => (
            <div key={roomType.id} className="room-type-card" style={{
              border: roomCounts[roomType.id] ? '2px solid var(--highlight)' : '1px solid var(--accent)',
              borderRadius: '4px',
              padding: '15px',
              ...
              <div style={{ marginBottom: '12px' }}>
                <h3 className="text-2d" style={{ 
                  margin: 0, 
                  fontSize: '18px',
                  color: roomCounts[roomType.id] ? 'var(--highlight)' : 'var(--text)'
                }}>
                  {roomType.name}
                </h3>
                <p className="text-2d" style={{ 
                  ...
                  {roomType.description || "A customizable room for your space."}
                </p>
              </div>
              
              <div className="room-count-controls" style={{ 
                display: 'flex',
                alignItems: 'center',
                ...
                <button 
                  ...
                  onClick={() => handleRoomCountChange(roomType.id, (roomCounts[roomType.id] || 0) - 1)}
                  disabled={!roomCounts[roomType.id] || roomCounts[roomType.id] === 0}
                >
                  −
                </button>
              ...
              {roomCounts[roomType.id] > 0 && (
                <div style={{ 
                  ...
                  {roomCounts[roomType.id]} {roomCounts[roomType.id] === 1 ? 'room' : 'rooms'} selected
                </div>
              )}
            </div>
          ))}
      ...
      {totalSelectedRooms > 0 && (
        <div style={{ 
          ...
          <p>Total rooms: <strong>{totalSelectedRooms}</strong></p>
      ...
      <div style={{ 
        ...
        <button 
          ...
          style={{ 
            ...
            fontSize: '16px',
            fontWeight: 'bold',
            background: totalSelectedRooms > 0 ? 'var(--highlight)' : 'var(--accent)', 
            color: 'var(--secondary)',
            ...
            borderRadius: '8px',
            border: 'none',
            cursor: totalSelectedRooms > 0 && !loading ? 'pointer' : 'not-allowed',
            boxShadow: totalSelectedRooms > 0 ? '0 4px 10px rgba(var(--highlight-rgb), 0.3)' : 'none',
            transition: 'all 0.2s ease'
          }}
        >
          {loading ? 'Creating Space...' : `Create Space (${totalSelectedRooms} rooms)`}
          {loading && (
            <span style={{
