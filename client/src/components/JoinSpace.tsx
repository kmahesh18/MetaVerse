...
export function JoinSpace() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:${import.meta.env.VITE_BKPORT}/api/spaces/${spaceId}/join`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Failed to join space: ${response.status}`);
      }
    ...
    <div className="container-2d" style={{ maxWidth: "600px", margin: "40px auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group" style={{ marginBottom: "20px" }}>
          <label htmlFor="spaceId" className="text-2d" style={{ display: "block", marginBottom: "5px", fontSize: "20px" }}>
            Space ID:
          </label>
          <input
            type="text"
