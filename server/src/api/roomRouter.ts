...
roomRouter.get("/:id/space", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `GET /api/rooms/${req.params.id}/space - Error:`,
      err.message,
    );
    res
      .status(500)
...
roomRouter.get("/:roomId/assets", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `GET /api/rooms/${req.params.roomId}/assets - Error:`,
      err.message,
    );
    res
      .status(500)
...
roomRouter.get("/:roomId/players", async (req, res) => {
  try {
    const { roomId } = req.params;
    const room = roomsById.get(roomId);
    const positionsObject = room?.playerPositions
...
roomRouter.get("/:roomId/userAvatars", async (req, res) => {
  const { roomId } = req.params;
  const clientAvatarsMap = await getRoomPlayersAvatars(roomId);
